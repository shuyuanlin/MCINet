import torch
import numpy as np
from HCF.common import rigid_transform_3d, transform


class DIS_method(torch.nn.Module):
    def __init__(self,
        ...
        super(DIS_method, self).__init__()
    ...
    def dynamic_inlier_selection(self, global_corr_scores,initial_trans, src_keypts, tgt_keypts, it_num, weights=None):
        """
        Perform post refinement using the initial transformation matrix with adaptive scaling.
        Based on Cauchy IRLS, adaptively update inlier weights and thresholds.
        Input
        ...
        # Set inlier thresholds based on dataset (e.g., 3DMatch or KITTI)
        ...
        for iteration in range(it_num):
            # Get the current inlier threshold for this iteration
            inlier_threshold = inlier_threshold_list[iteration]

            # Warp source keypoints with the initial transformation
            warped_src_keypts = transform(src_keypts, initial_trans)

            # Calculate L2 distance between warped and target keypoints
            L2_dis = torch.norm(warped_src_keypts - tgt_keypts, dim=-1)

            # Cauchy IRLS-inspired adaptive weight: lower residual -> higher weight
            adaptive_weights = 1 / (1 + (L2_dis / inlier_threshold) ** 2)
            adaptive_weights = adaptive_weights * global_corr_scores

            # Predicted inliers based on the current threshold
            pred_inlier = (L2_dis < inlier_threshold)[0]  # Assume bs = 1
            inlier_num = torch.sum(pred_inlier)

            # Stop refinement if the number of inliers does not change
            if abs(int(inlier_num - previous_inlier_num)) < 1:
                break
            else:
                previous_inlier_num = inlier_num

            # 保存当前旋转、当前变换、当前 inlier 和初始权重
            history_rotations.append(initial_trans[0, :3, :3].detach())
            transformation_list.append(initial_trans.clone())
            pred_inlier_list.append(pred_inlier)
            adaptive_weight_list.append(adaptive_weights.clone())

            # Refine the transformation using the current inliers and adaptive weights
            initial_trans = rigid_transform_3d(
                A=src_keypts[:, pred_inlier, :],
                B=tgt_keypts[:, pred_inlier, :],
                weights=adaptive_weights[:, pred_inlier],  # Use adaptive weights
            )

        num_iters = len(history_rotations)
        if num_iters >= 2:
            M = num_iters
            delta_list = []
            for k in range(1, M):
                delta = self.angle_between_rotations(history_rotations[k - 1], history_rotations[k])
                delta_list.append(delta)

            # 计算加权衰减项
            decay = torch.ones_like(global_corr_scores)
            for k, delta in enumerate(delta_list):
                g_k = 2 * (k + 1) / (M * (M + 1))  # g(m) = 2m / (M(M+1))
                decay *= torch.exp(-g_k * delta ** 2)

            final_weights = adaptive_weight_list[-1] * decay

            final_trans = rigid_transform_3d(
                A=src_keypts[:, pred_inlier_list[-1], :],
                B=tgt_keypts[:, pred_inlier_list[-1], :],
                weights=final_weights[:, pred_inlier_list[-1]],
            )
            return final_trans
        else:
    ...
    def angle_between_rotations(self,R1, R2):
        R = R1 @ R2.T
        trace = torch.clamp((R.trace() - 1) / 2, -1.0, 1.0)
    ...
    def iterative_pose_estimation(self, src_keypts, tgt_keypts, global_corr_scores):
        """
        ...
        #################################
        final_trans = self.dynamic_inlier_selection(global_corr_scores , final_trans, src_keypts, tgt_keypts, 20, None)
