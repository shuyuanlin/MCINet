...
class PAREConvFPN(nn.Module):
    def __init__(self, init_dim, output_dim, kernel_size, share_nonlinearity=False, conv_way='edge_conv', use_xyz=True, use_encoder_re_feats=True):
        ...
        # self.eaa = EfficientAdditiveAttention(3,3)
    ...
    def forward(self, data_dict):
        ...
        feats_s2 = self.encoder2_1(points_list[1], points_list[0], feats_s1, subsampling_list[0])  # 6660,32,3
        feats_s2 = self.encoder2_2(points_list[1], points_list[1], feats_s2, neighbors_list[1])  # 6660,64,3
        feats_s2 = self.encoder2_3(points_list[1], points_list[1], feats_s2, neighbors_list[1])  # 6660,64,3

        feats_s3 = self.encoder3_1(points_list[2], points_list[1], feats_s2, subsampling_list[1])  # 1753,128,3
        feats_s3 = self.encoder3_2(points_list[2], points_list[2], feats_s3, neighbors_list[2])  # 1753,128,3
        feats_s3 = self.encoder3_3(points_list[2], points_list[2], feats_s3, neighbors_list[2])  # 1753,128,3

        feats_s4 = self.encoder4_1(points_list[3], points_list[2], feats_s3, subsampling_list[2])  # 499,256,3
        feats_s4 = self.encoder4_2(points_list[3], points_list[3], feats_s4, neighbors_list[3])  # 499,256,3
        feats_s4 = self.encoder4_3(points_list[3], points_list[3], feats_s4, neighbors_list[3])  # 499,256,3


        coarse_feats = self.coarse_RI_head(feats_s4)  # 499,256,3
        ri_feats_c, _ = self.coarse_std_feature(coarse_feats)  # 499,256,3

        ri_feats_c = ri_feats_c.reshape(ri_feats_c.shape[0], -1)  # 499,783

        up1 = upsampling_list[1]  # 1753,1
        latent_s3 = index_select(feats_s4, up1[:, 0], dim=0)
        latent_s3 = torch.cat([latent_s3, feats_s3], dim=1)  # 1753,384,3
        latent_s3 = self.decoder3(latent_s3)  # 1753,128,3

        up2 = upsampling_list[0]
        latent_s2 = index_select(latent_s3, up2[:, 0], dim=0)
        latent_s2 = torch.cat([latent_s2, feats_s2], dim=1)
        latent_s2 = self.decoder2(latent_s2)  # 1660,85,3

        ri_feats = self.RI_head(latent_s2)  # 6660,85,3
        re_feats = self.RE_head(latent_s2)  # 6660,85,3

        ri_feats_f, local_rot = self.fine_std_feature(ri_feats)  # 6660,85,3
        ri_feats_f = ri_feats_f.reshape(ri_feats_f.shape[0], -1)  # 6660,255
        ...
        return re_feats_f, ri_feats_f, feats_s4, ri_feats_c, m_scores  # [6660,85,3][6660,255][499,256,3][499,768][6660]
