...
from HCF.hcf import HCF_module
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
def make_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument('--benchmark', default='3DLoMatch', choices=['3DMatch', '3DLoMatch'],  help='test benchmark')
    parser.add_argument('--method', default='svd', choices=['fhp', 'ransac', 'svd'], help='registration method')
...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_root in scene_roots:
        ...
        for file_name in file_names:
            ...
            # # print(ref_corr_points.shape)  #(2000,3)
            # # print(src_corr_points.shape)  #(2000,3)
            # # print(corr_scores.shape)  #(2000,)

            # ref_corr_points = torch.from_numpy(ref_corr_points)
            # src_corr_points = torch.from_numpy(src_corr_points)

            # ref_corr_points = torch.from_numpy(ref_corr_points).to(device)
            # src_corr_points = torch.from_numpy(src_corr_points).to(device)
            # src_corr_points = src_corr_points.unsqueeze(0)
            # ref_corr_points = ref_corr_points.unsqueeze(0)
            # hcf = HCF_module()
            # hcf.to(device)
            # pred_trans = hcf.hierarchical_corr_filtering(src_corr_points, ref_corr_points)
            # frag1_warp = Transform(src_corr_points, pred_trans)
            # distance = torch.sum((frag1_warp - ref_corr_points) ** 2, dim=-1) ** 0.5
            # pred_labels = (distance < 0.2).float()
            # _, row_index = torch.where(pred_labels)
            # src_corr_points = src_corr_points[:, row_index, :]
            # ref_corr_points = ref_corr_points[:, row_index, :]
            #
            # ref_corr_points = ref_corr_points.squeeze(0)
            # src_corr_points = src_corr_points.squeeze(0)
            #
            # src_corr_points = src_corr_points.cpu().numpy()
            # ref_corr_points = ref_corr_points.cpu().numpy()

            # src_corr_points = src_corr_points.numpy()
            # ref_corr_points = ref_corr_points.numpy()
...
def main():
    ...
    cfg = make_cfg()
    log_file = osp.join(cfg.log_dir, 'eval-ncmnet_Adap_GC2.log')
