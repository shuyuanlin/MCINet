...
from pareconv.modules.registration.registration import Dynamic_Inlier_Selection
from backbone import PAREConvFPN
from MCI_Net import MCI_Net
...
class PARE_Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.fine_matching = Dynamic_Inlier_Selection(
            cfg.fine_matching.topk,
            cfg.fine_matching.acceptance_radius,
            confidence_threshold=cfg.fine_matching.confidence_threshold,
            num_hypotheses=cfg.fine_matching.num_hypotheses,
            num_refinement_steps=cfg.fine_matching.num_refinement_steps,
            inlier_threshold=cfg.fine_matching.inlier_threshold,
        )

        self.point_matching = PointDualMatching(dim=cfg.backbone.output_dim // 3 * 3)

        self.mcinet = MCI_Net()
        self.m_weight = nn.Parameter(torch.ones([2]).float(), requires_grad=True)
    ...
    def forward(self, data_dict):
        ...
        # [6660,85,3][6660,255][499,256,3][499,768][6660]
        ...
        ref_feats_c_mci = self.mcinet(ref_feats_c.unsqueeze(0).unsqueeze(0))
        src_feats_c_mci = self.mcinet(src_feats_c.unsqueeze(0).unsqueeze(0))
        ref_feats_c = ref_feats_c + ref_feats_c_mci * self.m_weight[0]
        src_feats_c = src_feats_c + src_feats_c_mci * self.m_weight[1]
        ...
        # 6 Generate batched node points & feats 分别是选择的knn点、掩码、下标
